...
sudo docker build -f "Dockerfile" -t purnamasftr/hijub:frontend-$TRAVIS_BUILD_NUMBER  --label "Maintaner  Purnama Syafitri M <syafitri@alterra.id>" .
# push apps image to docker hub
sudo docker push purnamasftr/hijub:frontend-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta16 --kubeconfig kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=purnamasftr/hijub:frontend-${TRAVIS_BUILD_NUMBER}